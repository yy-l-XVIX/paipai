<?php
	#require
	require_once(dirname(dirname(dirname(dirname(__file__)))).'/inc/#Unload.php');
	require_once(dirname(dirname(dirname(dirname(__FILE__)))).'/inc/lang/'.$aSystem['sLang'].'/register.php');

	#require結束

	#給此頁運用的css，按先後順序排列陣列
	$aCss = array();
	#css 結束

	#給此頁運用的js，按先後順序排列陣列
	$aJs = array(
		'0'	=> 'plugins/js/register/register.js',
	);
	#js結束

	#參數接收區

	#參數結束

	#給此頁使用的url
	$aUrl   = array(
		'sAct'	=> sys_web_encode($aMenuToNo['pages/register/php/_register_0_act0.php']).'&run_page=1',
		'sHtml'	=> 'pages/register/'.$aSystem['sClientHtml'].$aSystem['nClientVer'].'/register_0.php',
	);
	#url結束

	#參數宣告區
	$nKid = 0;
	$sPromoCode = '';
	if (isset($aJWT['nKid']))
	{
		$nKid = $aJWT['nKid'];
	}
	if (isset($aJWT['sPromoCode']))
	{
		$sPromoCode = $aJWT['sPromoCode'];
	}
	$aData = array();
	$aValue = array(
		'a'		=> 'INS',
		'nExp'	=> NOWTIME + JWTWAIT
	);
	$sJWT = sys_jwt_encode($aValue);
	$aValue = array(
		'a'		=> 'GETVCODE',
	);
	$sVcodeJWT = sys_jwt_encode($aValue);

	$aSystem['aParam']['nSMSTime'] = 300;


	$aJumpMsg['0']['sMsg'] = '123';
	$aJumpMsg['0']['nClicktoClose'] = 1;
	$aJumpMsg['0']['aButton']['0']['sClass'] = 'JqClose';
	$aJumpMsg['0']['aButton']['0']['sText'] = CONFIRM;

	$aJumpMsg['1'] = $aJumpMsg['0'];
	$aJumpMsg['1']['nClicktoClose'] = 0;
	$aJumpMsg['1']['aButton']['0']['sClass'] = '';


	#宣告結束

	#程式邏輯區

	#程式邏輯結束

	#輸出json
	$sData = json_encode($aData);
	$aRequire['Require'] = $aUrl['sHtml'];
	#輸出結束
?>