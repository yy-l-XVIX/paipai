<?php
	...
	$aUrl   = array(
		...
		'sAct'	=> sys_web_encode($aMenuToNo['pages/center/php/_photo_0_act0.php']).'&run_page=1',
		...
		'sHtml'	=> 'pages/center/'.$aSystem['sClientHtml'].$aSystem['nClientVer'].'/photo_0.php',
	);
	#url結束
	...
	$sDelImgJWT=sys_jwt_encode($aValue);
	$nCount = 0;
	$nErr = 0;
	$sErrMsg = '';
	...
	$Result->bindValue(':sKid','%'.$aCodeLid['staff'].'%',PDO::PARAM_STR); // 人才有照片
	sql_query($Result);
	while ($aRows = $Result->fetch(PDO::FETCH_ASSOC))
	{
		$aData = $aRows;
	...
	else
	{
		...
		while ($aRows = $Result->fetch(PDO::FETCH_ASSOC))
		{
			$aData['aImage'][] = array(
				'nId' 	=> $aRows['nId'],
				'sImgUrl' 	=> IMAGE['URL'].'magic/resize/'.$aFile['sDir'].'/'.date('Y/m/d/',$aRows['nCreateTime']).$aRows['sTable'].'/'.$aRows['sFile'],
			);
		}
