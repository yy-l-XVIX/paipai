<?php
	...
	require_once(dirname(dirname(dirname(dirname(__FILE__)))).'/inc/lang/'.$aSystem['sLang'].'/photo.php');
	...
	if ($aJWT['a'] == 'UPT'.$aUser['nId'])
	{
		...
		if (isset($_FILES['sFile']) && $_FILES['sFile']['name']<>'')
		{
			...
			$nIsUpload ++;
			$aReturn['sMsg'] .= aPHOTO['HEADUPTV'].'<br>';

		}

		// 上傳圖片(multiple)
		if (!empty($_FILES['sFilePhoto']['name']))
		{
			# 檢查已上傳數量
			$sSQL = '	SELECT	1
					FROM	'.	CLIENT_IMAGE_CTRL .'
			...
			$Result->bindValue(':sTable', 'client_user_photo', PDO::PARAM_STR);
			sql_query($Result);
			$nCount = $Result->rowCount();
			if ($nCount >= $aSystem['aParam']['nPhotoLimit'])
			{
				$aReturn['sMsg'] .= aPHOTO['MAXLIMIT'].$aSystem['aParam']['nPhotoLimit'].'<br>';
				$aReturn['nStatus'] = 0;
				echo json_encode($aReturn);
				exit;
			}
			# file loop
			for ($i=0; $i < ($aSystem['aParam']['nPhotoLimit']-$nCount); $i++)
			{
				if (!isset($_FILES['sFilePhoto']['name'][$i]))
				{
					continue ;
				}
				if ($_FILES['sFilePhoto']['error'][$i] !== UPLOAD_ERR_OK)
				{
					continue ;
				}
				$aFile['sTable'] = 'client_user_photo';
				// $aFile['aFile'] = $_FILES['sFilePhoto'];
				$aFile['aFile'] = array(
					'name'	=> $_FILES['sFilePhoto']['name'][$i],
					'type'	=> $_FILES['sFilePhoto']['type'][$i],
					'tmp_name'	=> $_FILES['sFilePhoto']['tmp_name'][$i],
					'error'	=> $_FILES['sFilePhoto']['error'][$i],
					'size'	=> $_FILES['sFilePhoto']['size'][$i],
				);
				$aFileInfo = goImage($aFile);
				...
				{
					$aReturn['nStatus'] = 0;
					$aReturn['sMsg'] .= $_FILES['sFilePhoto']['name'][$i].' '.aIMGERROR[strtoupper($aFileInfo['error'])].'<br>';
					echo json_encode($aReturn);
					exit;
				}
				else
				...
				$aSQL_Array = array(
					'nKid'		=> (int) $aUser['nId'],
					'sTable'		=> (string) 'client_user_photo',
					'sFile'		=> (string) $sFname,
					'nType0'		=> (int) 0,
					'nCreateTime'	=> (int) NOWTIME,
					'sCreateTime'  	=> (string) NOWDATE,
			...
			$aReturn['sMsg'] .= aPHOTO['UPTV'].'<br>';
		}

		if (/*isset($_FILES['sFilePhoto']) && $_FILES['sFilePhoto']['name']<>'' */ false)
		{
			# 檢查上傳數量
			$sSQL = '	SELECT	1
					FROM	'.	CLIENT_IMAGE_CTRL .'
		...
		if ($aUser['nStatus'] == 11)
		{
			...
			if ($aUserPendingStatus[$aPendingField['sPhoto']] == 99) // 被拒絕更正資料
			{
				$aUserPendingStatus[$aPendingField['sPhoto']] = 0; // 改成未審核
			}
