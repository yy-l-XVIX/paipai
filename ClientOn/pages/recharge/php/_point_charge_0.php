<?php
	...
	#給此頁運用的js，按先後順序排列陣列
	$aJs = array(
		'0'	=> 'plugins/js/recharge/point_charge.js',
	);
	#js結束
	...
	$aUrl   = array(
		...
		'sAct'		=> sys_web_encode($aMenuToNo['pages/recharge/php/_point_charge_0_act0.php']).'&run_page=1',
		'sPage'		=> sys_web_encode($aMenuToNo['pages/recharge/php/_point_charge_0.php']),
		'sHtml'		=> 'pages/recharge/'.$aSystem['sClientHtml'].$aSystem['nClientVer'].'/point_charge_0.php',
	...
	if (!in_array('3', $aRechargeTunnel)) // 1:線上入款 2:公司入款 3:點數扣款
	{
		$nErr = 1;
		$sErrMsg = NODATA;
	}
	...
	{
		$aPayMethod[$LPnType] = aPOINTCHARGE['aPAYMETHOD'][$LPnType];
